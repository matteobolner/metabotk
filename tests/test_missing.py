...
import pandas as pd
import warnings
from tests.utils import create_test_dataframe_with_missing
...
class TestDetectMissingValues:
    ...
    def test_single_missing(self):
        ...
        assert np.array_equal(
            detect_missing_values(data), [False, False, False, True, False]
        )

    def test_multiple_missing(self):
        data = np.array([1, np.nan, 1, np.nan, 1, 2, 3])
        assert np.array_equal(
            detect_missing_values(data), [False, True, False, True, False, False, False]
        )
