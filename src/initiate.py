...
def split_data_metadata(input_data, metabolite_info, metabolite_id_column="CHEM_ID"):
    ...
    metadata_columns = [
        col for col in input_data.columns if col not in metabolite_names
    ]
    ...
    metabolites = metabolite_info[
        metabolite_info[metabolite_id_column].isin(metabolite_names)
    ]
...
class DataClass:
    ...
    def __init__(
        self, data, metabolite_info="config/metabolites.tsv", sample_info=None
    ):
        """
        ...
        data, metadata, metabolites = split_data_metadata(
            parsed_data, metabolite_info, "CHEM_ID"
        )
        self.metadata = metadata
        self.data = data
        self.metabolites = metabolite_info[
            metabolite_info["CHEM_ID"].isin(metabolites)
    ...
    def split_by_column(self, column):
        ...
        split_data = {
            name: DataClass(group, self.metabolites)
            for name, group in merged.groupby(by=column)
        }
        return split_data
    ...
    def drop_unused_metabolites(self):
        ...
        return self.metabolites[
            self.metabolites["CHEM_ID"].isin(self.data.columns)
        ].reset_index(drop=True)
    ...
    def update_data(self, new_data):
        ...
        if isinstance(new_data, pd.DataFrame):
            if self.check_if_same_length_of_data_and_metadata(new_data):
                ...
                raise ValueError(
                    "The number of rows in new data and metadata are different."
                )
        else:
    ...
    def split_metabolites_by_origin(self):
        ...
        for name, group in self.metabolites.groupby(by="ORIGIN"):
            temp_metabolites = self.metabolites[self.metabolites["ORIGIN"] != name][
                "CHEM_ID"
            ].tolist()
    ...
    def drop_exogenous_metabolites(self):
        ...
        """
        self.update_data(
            self.drop_metabolites(
                self.metabolites[self.metabolites["ORIGIN"] == "EXOGENOUS"]["CHEM_ID"]
            )
        )
    ...
    def get_outliers_matrix(self, threshold=5, binary=False):
        ...
        """
        matrix = self.data.copy().apply(
            lambda x: self.single_feature_outliers(x, threshold=threshold)
        )
    ...
    def drop_columns_with_missing_threshold(
        self, threshold=0.25, remove_also_outliers=True
    ):
        """
        ...
        if remove_also_outliers == False:
            ...
            missing = (
                self.feature_missingness().set_index("metabolite")["missing"]
                + self.feature_outliers().set_index("metabolite")["outliers"]
            )
        missing.index.name = None
        new_data = self.drop_metabolites(
            list(missing[(missing / len(self.data)) > threshold].index)
        )
    ...
    def feature_stats(self):
        ...
        metabolite_info = self.metabolites[
            ["CHEM_ID", "SUPER_PATHWAY", "SUB_PATHWAY", "CHEMICAL_NAME", "ORIGIN"]
        ]
